...
"""
Created on Thu Nov 25 13:02:25 2021
...
save_path=os.path.join(file_path,"backtest_results_mean_std_mean_log")
...
class sell_near_strategy(calender_strategy):
    ...
    def _get_mirror_map_sigma(self,df):
       df=df[df>=df.median()]
       df_map=2*df.min()-df
       df1=pd.concat([df,df_map])
       #check extreme value & replace
       upper=df1.mean()+3*df1.std()
       down=df1.mean()-3*df1.std()
       df1[df1>=upper]=upper
       df1[df1<=down]=down
       sigma=df1.std()
       return sigma
    ...
    def _get_daily_boundary(self):
        ...
        for date in self.trade_date:
            ...
            if start_date < np.min(self.trade_date):
                ...
                if self.use_mean:
                    df_sub1=df_sub[self.iv_ratio_type].copy()
                    #check extreme value & replace
                    upper=df_sub1.mean()+3*df_sub1.std()
                    down=df_sub1.mean()-3*df_sub1.std()
                    df_sub1[df_sub1>=upper]=upper
                    df_sub1[df_sub1<=down]=down
                    mu=df_sub1.mean()
                else:
                    mu=df_sub[[self.iv_ratio_type]].median()
                    
                if self.use_mirror_mapping:
                    sigma=self._get_mirror_map_sigma(df_sub[self.iv_ratio_type])
                else:
                    sigma=df_sub[self.iv_ratio_type].std()
...
def get_trade_info(upper,close,month,greek):
    ...
    if close <0 and abs(close)>=upper:
        ...
        strategy_call=sell_near_strategy(df_call_iv,df_call_iv_daily,"log_iv_ratio",call_atm,\
                                         upper,close,month,True,True,10,5)
...
if __name__== "__main__":
    ...
    l_upper=[1.1,1.2,1.3,1.4,1.6,1.7,1.8,1.9]
    ...
    def trade_success(prama_tuple,df_ret_call,df_ret_put,start_time="2017-01-01"):
        ...
        read_path_call_signal=os.path.join(signal_record_path_call,"signal_upper_sigma{}_close_sigma{}_roll_month{}_greek{}.csv".format(prama_tuple[0],prama_tuple[1],prama_tuple[2],prama_tuple[3]))
        read_path_put_signal=os.path.join(signal_record_path_put,"signal_upper_sigma{}_close_sigma{}_roll_month{}_greek{}.csv".format(prama_tuple[0],prama_tuple[1],prama_tuple[2],prama_tuple[3]))
        ...
        index_name="upper_sigma{}_close_sigma{}_roll_month{}_greek{}".format(prama_tuple[0],prama_tuple[1],prama_tuple[2],prama_tuple[3])
        ...
        total_call=len(df_signal_call[df_signal_call["open"]>=start_time])
        total_put=len(df_signal_put[df_signal_put["open"]>=start_time])
    ...
    for idx,arg in enumerate(l_pool_arg):
        param_name="upper_sigma{}_close_sigma{}_roll_month{}_greek{}".format(arg[0],arg[1],arg[2],arg[3])
        arg_rst=rst[idx]
